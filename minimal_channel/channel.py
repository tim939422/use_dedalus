...
WSS = d3.Average(nu*dz(u@ex),('x', 'y'))
TWSS = d3.Interpolate(WSS, 'z', 'right')
BWSS = d3.Interpolate(WSS, 'z', 'left')
...
u['g'][0] += 1.5*(1 - z**2)
# faster transition
u['g'][1] += 0.4*u['g'][0]*np.cos(16*np.pi*x/Lx)
u['g'][0] += 0.4*u['g'][0]*np.cos(16*np.pi*y/Ly)
...
checkpoints = solver.evaluator.add_file_handler('checkpoints', sim_dt=100, max_writes=1, mode='append')
...
flow.add_property(np.abs(u@ex), name='U')
flow.add_property(np.abs(u@ey), name='V')
flow.add_property(np.abs(u@ez), name='W')
flow.add_property(0.5*(np.abs(BWSS) + np.abs(TWSS)), name='WSS')
...
try:
    ...
    while solver.proceed:
        ...
        if (solver.iteration-1) % 10 == 0:
            ...
            Wmax = flow.max('W')
            tau_w = flow.max('WSS')
            logger.info(f'Iteration={solver.iteration:d}, Time={solver.sim_time:e}, dt={timestep:e}, Umax={Umax:f}, Vmax={Vmax:f}, Wmax={Wmax:f}, Wall Shear={tau_w:e}')
except:
